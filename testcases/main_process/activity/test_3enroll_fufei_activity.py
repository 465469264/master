...
from api.app.selActOrderInfoByOrderNo import selActOrderInfoByOrderNo
from api.app.payUpwardActivityFee import payUpwardActivityFee
...
class Test_enroll_fufei_activity(HttpRunner):
    config = (
        Config("生成活动付费订单-缴费")
            ...
            .variables(**{
                        ...
                        "orderNo": "165693114182835736",               #活动的固定orderNo
                        "message": "success"
    ...
    teststeps = [
        ...
        Step(RunTestCase("获取用户信息，获取userId").setup_hook('${delay(1)}').call(get_info).teardown_hook('${delete_activity($userId)}').export(*["userId"])),
        ...
        Step(RunTestCase("获取智米审核web_token").setup_hook('${delay(1)}').call(zhimi_check_token).teardown_hook('${get_html($body)}',"_web_token").export(*["_web_token"])),
        Step(RunTestCase("智米赠送").call(check_zhimi)),
        Step(RunTestCase("根据orderNO 生成订单信息").call(selActOrderInfoByOrderNo)),
        Step(RunTestCase("用智米抵扣支付").with_variables(**({"amount": "0.00",  "zmDeduction": "100", "accDeduction": "0.0","orderNo": "$body"})).call(payUpwardActivityFee)),
