...
class Test_Apply_qita(HttpRunner):
    ...
    teststeps = [
        ...
        Step(RunTestCase("申请证明").with_variables(**({"checkStatus":"","stdName": "$std_name", "userId": "$unvsId", "applyType": "5", "remark": "测试", "applyPurpose": "测试","receiveType": "1", "unvsName": "",
                                                    "district": "$districtName", "city": "$cityName",
                                                     "stampDown": "1", "receiveName": "测试","province": "$provinceName",
                                                    "receiverName": "测试","receiverMobile": "$mobile",
                                                    "receiveMobile": "$mobile", "receiveAddress": "北京朝阳区三环以内测试",
                                                    "receiverAddress": "北京朝阳区三环以内测试","materialName": "测试",
                                                    })).call(getCertificateApply)),

        Step(RunTestCase("查询我的申请列表第一条是审核中").with_variables(**({"checkStatus":"0","stdName": "", "userId": "", "applyType": "3", "remark": "", "applyPurpose": "",
                                                             "receiveType": "", "unvsName": "","grade":"",
        ...
        Step(RunTestCase("审核学员证明为不通过").with_variables(**({"_web_token":"","checkStatus":"-1","reason":"您申请的证明材料不在受理范围内","applyTyp":"$applyType"})).call(fdyCheck)),
