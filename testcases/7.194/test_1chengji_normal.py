...
class TestCasesActivity_sendAppMsg(HttpRunner):
    ...
    teststeps = [
        ...
        Step(RunTestCase("获取学生第一期-科目1成绩").with_variables(**({"semester":"1"})).call(findStudentTScoreBySemester).export(*["courseName","courseId",
                                                                                                 "totalmark","totalRewardScore","teacher","teacherId","advScore","usualTimeMark"])),
        Step(RunTestCase("编辑第一学期-科目1成绩-常规课程有卷面分，学业奖励分20-正常状态").with_variables(**({"a":"0","score":"50","rewardScore":"20",
                                                "examStatus":"4","courseScoreType":"1","examSubjectName":"$courseName"})).call(updateStudentTScore)),
        ...
        Step(RunTestCase("获取学生第一期-科目1成绩").with_variables(**({"semester": "1"})).call(findStudentTScoreBySemester).export(*["courseName","courseId",
                                                                                                 "totalmark","totalRewardScore","teacher","teacherId","advScore","usualTimeMark"])),
        Step(RunTestCase("编辑第一学期-科目1成绩-常规课程有卷面分，正常状态").with_variables(**({"a":"0","score": "50", "rewardScore": "", "examStatus": "4", "courseScoreType": "1",
                                                                                                                    "examSubjectName": "$courseName"})).call(updateStudentTScore)),
        Step(RunTestCase("修改后获取学生第一期-科目1成绩用来与APP的成绩对比").call(findStudentTScoreBySemester).export(*["score", "rewardScore", "totalRewardScore"])),
        ...
        Step(RunTestCase("获取编辑学生成绩的web_token").call(studentTScore_edit_token).teardown_hook('${get_html($body)}',"_web_token").export(*["_web_token"])),
        Step(RunTestCase("获取学生第一期-科目2成绩").with_variables(**({"semester": "1"})).call(findStudentTScoreBySemester).export(*["courseName1","courseId1","courseName1","advScore1",
                                                                                "usualTimeMark1","totalmark1","totalRewardScore1","teacherId1","teacher1"])),
        ...
        Step(RunTestCase("修改后获取学生第一期-科目2成绩用来与APP的成绩对比").call(findStudentTScoreBySemester).export(
            *["score1", "rewardScore1","totalRewardScore1"])),
        Step(RunTestCase("对比学生科目2成绩").with_variables(**({"a":"1","isPass": int("2"), "totalScore": "$totalRewardScore"})).call(selStdAchievement)),
