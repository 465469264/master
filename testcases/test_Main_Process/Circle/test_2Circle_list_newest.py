...
class Test_Circle_List_New(HttpRunner):
    config = (
        Config("圈子-精选最新列表，进入，评论，点赞，生成海报")
            .verify(False)
            .variables(**{
    ...
    teststeps = [
        ...
        Step(RunTestCase("查看圈子列表-精选最新-提取第一条帖子id").with_variables(**({"userRoleType": "$userCircleRole"})).call(SocialCircle_list).export(*["id"])),
        Step(RunTestCase("精选-最新-列表-第一条帖子详情").call(SocialCircle_CircleDetail).teardown_hook('${judge_fabulousNum($fabulous)}', "fabulousNum").export(*["fabulousNum", "userId"])),
        Step(RunTestCase("帖子的点赞列表").call(SelPraiseList)),
        Step(RunTestCase("若用户还没第一条帖子点赞，则点赞，否则取消点赞").with_variables(**({"fabulousNum": "$fabulousNum", "praiseId": "$id", "praiseType": "3"})).call(usPraise)),
...
if __name__ == "__main__":
    Test_Circle_List_New().test_start()