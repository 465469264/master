...
class Test_Circle_List_Run(HttpRunner):
    ...
    teststeps = [
        ...
        Step(RunTestCase("跑步习惯-获取习惯打卡列表下的封面，参与人头像，参与人数").call(selClockTaskTabInfos)),
        Step(RunTestCase("跑步习惯-跑步习惯列表").call(selClockTaskByType).teardown_hook('${get_task_id($body)}', "taskId").export(*["taskId"])),
        Step(RunTestCase("跑步习惯-查看习惯详情").call(selClockTaskDetails)),
        Step(RunTestCase("跑步习惯-习惯帖子列表-提取出第一个帖子id").with_variables(**({"userRoleType": "$userCircleRole"})).call(SocialCircle_habitList).export(*["id"])),
